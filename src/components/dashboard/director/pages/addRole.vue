<template>
    <div>
        ...
        <div class="card m-b-30">
            <div class="card-body">
                <p class="cv1">
                    Add Audition Role to <b class="col-ppd">{{detailsData.data.project_title}}</b>
                </p>
                <form class="msform mt-2" @submit.prevent="addRole">
                    <div id="room_fileds">
                        <b>Role name:</b>
                        ...
                        <input class="mb-2" type="file" @change="processBible($event)" required>

                        <b>Attach Audition script:</b>
                        <input class="mb-2" type="file" @change="processScript($event)" required>
    ...
    export default {
        ...
        methods: {
            processBible(event) {
                ...
                this.characterbible = event.target.files[0];
            },
            processScript(event) {
                event.target.files[0]
                this.script = event.target.files[0];
            },
            addRole() {
                ...
                axios.post(API_URL + '/project/addrole/' + userID + '/' + projectID, form).then(result => {
