            ...
            <div class="card-body">
                <p class="cv1">
                    <b class="col-ppd">Notification Settings</b>
                     <router-link class="mdb float-right" v-bind:to="'/director/dirNotifications'">View Notifications</router-link>
                </p>
                <div class="mt-2">
                   <form>
                    <div class="row">
...
export default {
	...
	mounted() {
		...
		axios({ method: 'GET', url: 'https://api.cast.i.ng/auditions/applied/' + userID, config }).then(
			result => {
				this.loading = false;
		...
		axios({ method: 'GET', url: 'https://api.cast.i.ng/qualifiedproject/' + userID, config }).then(
			result => {
