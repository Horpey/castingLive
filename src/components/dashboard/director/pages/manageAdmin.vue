...
export default {
	...
	mounted() {
		...
		axios({ method: 'GET', url: 'https://api.cast.i.ng/director/subadmins/' + userID, config }).then(
			result => {
				this.loading = false;
	...
	methods: {
		deleteAdmin(adminID) {
			...
			axios({ method: 'GET', url: 'https://api.cast.i.ng/delete/user/' + adminID, config }).then(
				result => {
					this.loading = false;
