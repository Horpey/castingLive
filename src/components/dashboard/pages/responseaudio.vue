				...
				<div id="nav-tabContent" class="tab-content py-3 px-3 px-sm-0">
					<div id="nav-auditions" role="tabpanel" aria-labelledby="nav-auditions-tab" class="tab-pane fade show active">
						<div class="mt-2">
							...
							<form class="msform mt-2" @submit.prevent="sendAudio">
								...
								<div id="room_fileds">
									...
									<span class="bmd-form-group"><input class="mb-2" type="text" placeholder="Audio URL" v-model="link" required></span>
								</div>
...
export default {
	...
	methods: {
		sendAudio() {
			...
			axios.post(API_URL + '/audition/addaudio/' + projectID, form).then(
				result => {
